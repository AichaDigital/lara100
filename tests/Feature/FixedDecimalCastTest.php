...
it('stores the unscaled integer and reads back a FixedDecimal at the declared scale', function () {
    ...
    expect($fresh->fd_amount)->toBeInstanceOf(FixedDecimal::class)
        ->and($fresh->fd_amount->toDecimalString())->toBe('19.99')
        ->and($fresh->fd_amount->scale())->toBe(2)
        ->and($fresh->fd_rate->toDecimalString())->toBe('21.0000')
        ->and($fresh->fd_rate->scale())->toBe(4);
});
